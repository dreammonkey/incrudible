import { getAdmins } from '@/Incrudible/Api/Admins'
            ...
            {actions.map((action, index) => (
              <DropdownMenuItem key={index}>
                <Link
                  ...
                  href={
                    action.route ? route(action.route, { admin: item.id }) : '#'
                  }
                >
...
export default function AdminIndex({ auth }: PageProps) {
  ...
  const {
    ...
    queryFn: () => getAdmins(baseRoute, filters),
  })
  ...
  return (
    <AuthenticatedLayout
      ...
      header={
        <>
          <h2 className="xs:ml-2 text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
            Admins
          </h2>
          <form
